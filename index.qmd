...
<div class="intro-modal" id="intro-modal" aria-hidden="true">
  ...
  <div class="intro-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="intro-modal-title">
    <button type="button" class="intro-modal-close" id="intro-modal-close" aria-label="Close">×</button>
...
dead_links_count <- sum(tolower(trimws(as.character(data$`Meta URL Status`))) == "dead", na.rm = TRUE)
...
::: {.metric-card}
<div class="metric-label">Sheet Last Modified</div>
<div class="metric-detail" id="sheet-last-modified">`r sheet_last_modified`</div>
:::

::: {.metric-card}
<div class="metric-label">Latest Row Created</div>
<div class="metric-detail" id="latest-row-created">`r latest_row_created`</div>
:::

::: {.metric-card}
<div class="metric-label">Latest Row Updated</div>
<div class="metric-detail" id="latest-row-updated">`r latest_row_updated`</div>
:::

::: {.metric-card}
<div class="metric-label">Dead Links</div>
<div class="metric-value">`r dead_links_count`</div>
:::
...
url_status_chip <- function(x) {
  ...
  label <- dplyr::case_when(
    status == "ok" ~ "Link Check: OK",
    status == "dead" ~ "Link Check: Dead",
    status == "warn" ~ "Link Check: Warn",
    TRUE ~ "Link Check: Unknown"
  )
  paste0(
...
data_display <- data %>%
  mutate(
    ...
    Details = paste0(
      ...
      vapply(seq_along(Title), function(i) detail_row_if_present("Link Check Status", `Meta URL Status`[[i]]), character(1)),
      vapply(seq_along(Title), function(i) detail_row_if_present("Link Check Time", `Meta URL Checked At`[[i]]), character(1)),
      vapply(seq_along(Title), function(i) detail_row_if_present("Link Check Detail", `Meta URL Check Detail`[[i]]), character(1)),
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!latestCreatedEl && !latestUpdatedEl && !sheetModifiedEl) return;
  ...
  fetch(GVIZ_URL)
    ...
    .then(function (text) {
      ...
      if ((createdIdx < 0 || updatedIdx < 0) && rows.length > 0) {
        ...
        if (fallbackCreatedIdx >= 0 || fallbackUpdatedIdx >= 0) {
