...
<form id="resource-form" class="submission-card" novalidate>
  ...
  <div class="section-block">
    ...
    <div class="form-grid">
      <div class="form-group">
        <label for="type">Type <span class="info-icon" tabindex="0" data-tooltip="The 'type' of the entry. You may select more than one." aria-label="More information about Type">i</span></label>
      ...
      <div class="form-group">
        <label for="topic">Topic <span class="info-icon" tabindex="0" data-tooltip="Topics covered by the asset. Select all that apply." aria-label="More information about Topic">i</span></label>
        <details class="multi-select-dropdown" open>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function parseGviz(text) {
    ...
    const columns = (table.cols || []).map(function (col) {
  ...
  function validateRecord(record) {
    ...
    const urlCheck = getUrlValidationState(trimValue("urls"));
    if (urlCheck.state === "missing") {
      return { ok: false, message: "Please enter at least one URL." };
    }
    if (urlCheck.state === "invalid") {
      const firstInvalid = urlCheck.invalid && urlCheck.invalid.length > 0 ? urlCheck.invalid[0] : "URL";
      return { ok: false, message: "Invalid URL detected: " + firstInvalid + ". Use full http(s) URLs with a public domain (e.g., .org, .com)." };
    }

    for (let i = 0; i < optionBindings.length; i += 1) {
      const binding = optionBindings[i];
      const otherValue = trimValue(binding.otherInputId);
      if (hasOtherSelected(binding.fieldId) && !otherValue) {
        const label = binding.optionKey;
        return { ok: false, message: "Please fill '" + label + " -> Other' details." };
      }
    }

    if (record["Contributor Email"]) {
      const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailPattern.test(record["Contributor Email"])) {
        return { ok: false, message: "Contributor Email is not valid." };
      }
    }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (missingRecommended.length > 0) {
      setStatus("Recommended fields are missing: " + missingRecommended.join(", ") + ". Submission can continue.", "status-warning");
    }
    ...
    try {
      ...
      setStatus("Submission sent successfully. Thank you for contributing.", "status-success");
      form.reset();
      initializeOtherInputToggles();
    } catch (error) {
      setStatus(error.message || "Submission failed.", "status-error");
    } finally {
      setSubmitting(false);
    }
  });
...
::: {.last-updated}
If you are setting this up for the first time, follow the Apps Script instructions in `README.md`.
:::
