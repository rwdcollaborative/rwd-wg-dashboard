...
```{r token-setup}
#| include: false
submit_token_js <- Sys.getenv("SUBMIT_TOKEN", "")
submit_token_js <- gsub("\\\\", "\\\\\\\\", submit_token_js)
submit_token_js <- gsub("\"", "\\\\\"", submit_token_js)
```
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateRecord(record) {
    ...
    if (urlCheck.state === "invalid") {
      ...
      return {
        ok: false,
        message: "Invalid URL detected: " + firstInvalid + ". Use full http(s) URLs with a public domain (e.g., .org, .com)."
      };
    }

    const email = trimValue("contributor-email");
    if (email) {
  ...
  async function submitRecord(record) {
    ...
    const payload = {
      ...
      submit_token: CONFIG.submitToken || "",
      record: record
    };
    ...
    if (!response.ok || body.ok === false) {
      ...
      if (code === "UNAUTHORIZED") {
        throw new Error("Submission is not authorized. Please contact the dashboard administrator.");
      }
      if (code === "RATE_LIMITED") {
        throw new Error(body.message || "Too many submissions right now. Please try again shortly.");
      }
      if (code === "DUPLICATE_RECENT") {
        throw new Error("This appears to be a recent duplicate submission.");
      }
      if (code === "VALIDATION_ERROR") {
        throw new Error(body.message || "Submission contained invalid values. Please review and retry.");
      }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      initializeOtherInputToggles();
      updateRecommendationHints();
      let successMessage = "Your resource was submitted successfully. Thank you for contributing.";
      if (result && Array.isArray(result.modified_fields) && result.modified_fields.length > 0) {
        const names = result.modified_fields
          .map(function (item) { return item && item.field ? item.field : ""; })
          .filter(Boolean)
          .slice(0, 5);
        const suffix = names.length > 0 ? names.join(", ") : "one or more fields";
        successMessage += " Note: the server normalized these field(s): " + suffix + ".";
      }
      openSubmissionModal({
